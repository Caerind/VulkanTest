...
void split(const std::string& base, std::vector<std::string>& result, char separator)
{
	U32 lastFound = 0;
	U32 found = base.find(separator);
	while (found != std::string::npos)
	{
		std::string str = base.substr(lastFound, found);
		if (str.size() > 0)
		{
			result.push_back(str);
		}
		lastFound = found + 1;
		found = base.find(separator, lastFound);
	}
	return;
}

void split(const std::string& base, std::vector<std::string>& result, const std::string& separator)
{
	U32 lastFound = 0;
	U32 found = base.find(separator);
	while (found != std::string::npos)
