...
{
	return Vector2<T>(v1.data[0] >= v2.data[0] ? v1.data[0] : v2.data[0], v1.data[1] >= v2.data[1] ? v1.data[1] : v2.data[1]);
}
...
{
	template<typename T>
	struct hash<nu::Vector2<T>>
	{
		size_t operator()(const nu::Vector2<T>& v) const;
	};

	template<typename T>
	size_t hash<nu::Vector2<T>>::operator()(const nu::Vector2<T>& v) const
	{
		size_t seed = 0;
		hash<T> hasher;
		nu::hashCombine(seed, hasher(v.x));
		nu::hashCombine(seed, hasher(v.y));
		return seed;
	}
}
