...
{
	template<typename T>
	struct hash<nu::Vector4<T>>
	{
		size_t operator()(const nu::Vector4<T>& v) const;
	};

	template<typename T>
	size_t hash<nu::Vector4<T>>::operator()(const nu::Vector4<T>& v) const
	{
		size_t seed = 0;
		hash<T> hasher;
		nu::hashCombine(seed, hasher(v.x));
		nu::hashCombine(seed, hasher(v.y));
		nu::hashCombine(seed, hasher(v.z));
		nu::hashCombine(seed, hasher(v.w));
		return seed;
	}
}
